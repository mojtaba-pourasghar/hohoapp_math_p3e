...
import com.hoohoomath.app.data.QuizResult;
...
public class ParentPanelFragment extends BaseFragment {
    ...
    private List<String> buildInsights(List<QuizResult> history) {
        ...
        if (worstChapter >= 0 && worstRatio < 0.7f) {
            ...
            tips.add("در فصل " + ch.numberFa + " (" + ch.title + ")، " + fa(Math.round(worstRatio * 100)) + "٪ پاسخ‌ها درست بوده. پیشنهاد: کاربرگ تمرینی همین فصل را با او کار کنید.");
        } else {
    ...
    private View buildResultRow(QuizResult r) {
        ...
        col.addView(UiKit.text(requireContext(), modeTitle + " — فصل " + ch.numberFa + ": " + ch.title, 13f, R.color.text_primary, true));
